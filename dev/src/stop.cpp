...
int main(int argc, char const *argv[])
{
 ...
 //    }
  ...
  while(1)
  {
	...
	cv::inRange(HSV, Scalar(160, 100, 160), Scalar(180, 255, 255), thd_red);
	...
	for( int i = 0; i< contours.size(); i=hierarchy[i][0] ) // iterate through each contour.
       {   
        	...
        	if(hierarchy[i][2]>0)
        	{
              ...
              approxPolyDP(contours[i],approxCircles222,arcLength(Mat(contours[i]), true)*0.01, true);
              std::cout<<"red, contoure size = "<< contours.size()
              <<" index = " << i
              <<"vertex =" << approxCircles222.size()
              <<std::endl;
        	 }
          }
          else 
